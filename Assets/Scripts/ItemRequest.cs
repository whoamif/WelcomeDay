...
public class ItemRequest : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Inrange && !Opened)
        {
            if (PlayerController.Instance.GetComponent<PlayerController>().CurentItem != null && ItemName == PlayerController.Instance.GetComponent<PlayerController>().CurentItem.ItemName)
            {
                if (GetComponent<Collider>().enabled)
                {
                    ...
                    if (Input.GetKeyDown(KeyCode.E))
                    {
                        ...
                        if (SoundEffect1 != null)
                            SoundEffect1.Play();
                        if (UseDestroy)
                            Destroy(PlayerController.Instance.GetComponent<PlayerController>().CurentItem.gameObject);
